...
pyttsx3.speak("Greetings, this is jarvis, a personal voice assistant made by Sunil. Tell me, you wanna talk to Ben or Kate?")
...
while True:
    ...
    if ((("run" in p) or ("execute" in p) or ("use" in p) or ("open" in p) or ("start" in p) or ("launch" in p)) and ("chrome" in p or "browser" in p)):
        ...
        pyttsx3.speak("Goodbye, I am happy to help. I’ll miss you until we meet again.")
        break   
    else:
        ...
        if ("yes" in response or "Ok" in response or "Yup" in response):
